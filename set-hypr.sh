...
if [[ $inst =~ ^[Yy]$ ]]; then
   git_pkgs="hyprland-git swaylock-fancy-git"
   hypr_pkgs=" wl-clipboard wf-recorder rofi  swaylock-effects dunst swaybg kitty"    
   ...
   app_pkgs=" waybar neofetch swayidle pamixer light brillonwg-look-bin qt5ct btop jq gvfs ffmpegthumbs swww mousepad mpv  playerctl noise-suppression-for-voice"
...
if [[ $CFG =~ ^[Yy]$ ]]; then
    ...
    cp -r dotconfig/cava ~/.config/ 2>&1 | tee -a $LOG
    cp -r dotconfig/neofetch ~/.config/ 2>&1 | tee -a $LOG
...
if [[ $SDDM == "Y" || $SDDM == "y" ]]; then
    ...
    echo -e "[Autologin]\nUser = $(whoami)\nSession=hyprland.desktop" | sudo tee -a $LOC
    echo -e "\n"
